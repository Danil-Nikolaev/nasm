...
; часть кода, которая будет на плюсах
	...
	mov rsi, input_str ; Address of input buffer
	mov rdx, str_len ; Maximum length to read
	syscall
	
	call PrintInputLetMsg
	
	; ввод сочетания
	mov rax, 0 ; System call 0 for read
	mov rdi, 0 ; File descriptor for stdin
	mov rsi, input_let ; Address of input buffer
	mov rdx, let_len ; Maximum length to read
	syscall
	
	mov rsi, input_let ; ввденное сочетание
	
	; подсчет длины введенного сочетания
	mov rcx, 22
	cycle_len:
		mov al, [rsi]
		cmp al, 0xA
